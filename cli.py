...
root_io_dir = Path(
    ...
    "/Users/bsh/brett/sortition/foundation/projects-events/Stratification-Services/python/example-input-output/"
...
# OUTPUT FILES:
people_selected_file_path = root_io_dir / "example_people_20-selected.csv"
people_remaining_file_path = root_io_dir / "example_people_20-remaining.csv"
...
def main():
    output_lines = []
    min_max_people = {}
    id_column, columns_to_keep, check_same_address, max_attempts = initialise_settings()
    ...
    with open(people_file_path, "r") as people_file:
        people, columns_data = init_categories_people(people_file, id_column, categories, columns_to_keep)

    success, tries, people_selected, output_lines = run_stratification(categories, people, columns_data, number_people_wanted, min_max_people, max_attempts, check_same_address)
    ...
    if success:
        ...
        with open(people_selected_file_path, mode="w") as selected_file, open(people_remaining_file_path, mode="w") as remaining_file:
            write_selected_people_to_file(people, people_selected, id_column, categories, columns_to_keep, columns_data, selected_file, remaining_file)
    print("\n".join(output_lines) + "\n")
